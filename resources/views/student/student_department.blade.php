...
<div class="row" style="font-size:36px;font-family:Microsoft JhengHei;">
  ...
  <div class="col text-center" style="background-color:rgb(33,66,109);">
    <a href="student_department?value=學士班(日間)&year={{$nowyear}}" style="color:white;" >系</a>
  </div>
</div>
</div>
<script>
...
var value="學士班(日間)";
location.href="student_department?year={{$nowyear}}&value=" +value;
}
...
(function initCharts() {
    var maxAttempts = 50; // 50 * 100ms = 5秒
    var attempts = 0;
    ...
    function tryInit() {
        ...
        if (typeof Highcharts === 'undefined') {
            if (attempts >= maxAttempts) {
                console.error('Highcharts 超時未載入，請檢查網路連線或刷新頁面');
                alert('Highcharts 圖表庫載入失敗，請刷新頁面重試。');
                return;
            }
            console.log('Waiting for Highcharts... (' + attempts + '/' + maxAttempts + ')');
            setTimeout(tryInit, 100);
            return;
        }
        
        console.log('Highcharts loaded, initializing charts...');
