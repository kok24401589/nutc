@extends("temp")
@section("title")
@if(is_null($department))
    {{$nowyear}}學年度入學管道分析-校(詳細資料)
@else
    {{$nowyear}}學年度入學管道分析-系(詳細資料)
@endif
@stop

{{-- ✅ 菜單連結已由 MenuComposer 統一處理，無需在此定義 --}}

@section("link")
    <script src="https://code.highcharts.com/modules/data.js"></script>
    <script src="https://code.highcharts.com/modules/heatmap.js"></script>
    <script src="https://code.highcharts.com/modules/treemap.js"></script>
    <style>
        .progwidth {
            max-width: 550px;
        }
        .treemapwidth{
            max-width: 600px;
        }     
        @media screen and (max-width: 766px) {
            .progwidth {
                max-width: 85%;
            }
            .treemapwidth {
                max-width: 100%;    
            }
        }
    </style>
@stop
@section("content")

<table width=100% border="1">
    <tr align="center" style="font-size:36px;font-family:Microsoft JhengHei;">
        @if(is_null($department))
            <td style="background-color:rgb(33,66,109);border:1px solid rgb(33,66,109);">
                <a style="color:white;">校</a></td>
            <td>
                <a href="EnrollmentAnalysisDepartment?value=資訊工程系&year={{$nowyear}}">系</a>
            </td>
        @else
            <td><a href="EnrollmentAnalysis?year={{$nowyear}}">校</a></td>
            <td style="background-color:rgb(33,66,109);border:1px solid rgb(33,66,109);">
                <a style="color:white;">系</a>
            </td>
        @endif
    </tr>
</table>
<div id="container" style="min-width: 200px;max-width: 600px;margin: 0 auto;"></div>
<div class='row'>
    <div class='col-md-3'>
        <div style='padding:5px;'><span style='font-size:40px'>技優甄審：{{$Skills_Screening->count()}}間</span><br></div>
        <table class='table table-striped'>
            @foreach($Skills_Screening as $S)
                <tr>
                    <td><span style='font-size:16px;color:#999999'>{{$loop->index+1}}</span></td>
                    <td>{{$S->入學前學校}}</td>
                    <td style='font-size:20px;white-space:nowrap;'>{{$S->人數}}名</td>
                </tr>
            @endforeach
        </table>
    </div>
    <div class='col-md-3'>
        <div style='padding:5px;'><span style='font-size:40px'>申請入學：{{$Admission->count()}}間</span><br></div>
        <table class='table table-striped'>
            @foreach($Admission as $A)
                <tr>
                    <td><span style='font-size:16px;color:#999999'>{{$loop->index+1}}</span></td>
                    <td>{{$A->入學前學校}}</td>
                    <td style='font-size:20px;white-space:nowrap;'>{{$A->人數}}名</td>
                </tr>
            @endforeach
        </table>
    </div>
    <div class='col-md-3'>
        <div style='padding:5px;'><span style='font-size:40px'>甄選入學：{{$Selection_admission->count()}}間</span><br></div>
        <table class='table table-striped'>
            @foreach($Selection_admission as $S)
                <tr>
                    <td><span style='font-size:16px;color:#999999'>{{$loop->index+1}}</span></td>
                    <td>{{$S->入學前學校}}</td>
                    <td style='font-size:20px;white-space:nowrap;'>{{$S->人數}}名</td>
                </tr>
            @endforeach
        </table>
    </div>
    <div class='col-md-3'>
        <div style='padding:5px;'><span style='font-size:40px'>其他：{{$Other->count()}}間</span><br></div>
        <table class='table table-striped'>
            @foreach($Other as $O)
                <tr>
                    <td><span style='font-size:16px;color:#999999'>{{$loop->index+1}}</span></td>
                    <td>{{$O->入學前學校}}</td>
                    <td style='font-size:20px;white-space:nowrap;'>{{$O->人數}}名</td>
                </tr>
            @endforeach
        </table>
    </div>
</div>
@stop